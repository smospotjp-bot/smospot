...
interface Props {
  /** When provided, the map opens here and searches immediately (area pages). */
  initialCenter?: LatLng;
  /** Visible H1 for the page (SEO). */
  heading: string;
}

export function SmoSpotApp({ initialCenter, heading }: Props) {
  ...
  // On mount: area pages search their fixed center; home asks for GPS.
  useEffect(() => {
    if (initialCenter) runSearch(initialCenter);
    else handleLocate();
        ...
        <div className="mb-4 flex items-center justify-between gap-2">
          <div className="flex min-w-0 items-center gap-2.5">
            ...
            <div className="min-w-0 leading-none">
              ...
              <h1 className="mt-1 truncate text-[11px] font-medium text-brand-pale/80">
                {heading}
              </h1>
            </div>
          </div>
          {spots.length > 0 && (
            <span className="shrink-0 rounded-full bg-white/15 px-3 py-1 text-xs font-bold text-white backdrop-blur-sm">
              {spots.length}件
            </span>
          )}
        </div>
...
function Logo() {
  return (
    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-white/15 ring-1 ring-white/25 backdrop-blur-sm">
